...
export default function Home() {
  ...
  const loadData = async (userAddress: string) => {
    ...
    try {
      ...
      // 컨트랙트 정보 조회
      const [name, symbol, balanceOf, owner] = await Promise.all([
        contract.name(),
        contract.symbol(),
      ...
      setContractInfo({ name, symbol, owner })
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-lg p-6 mb-6 border border-zinc-200 dark:border-zinc-800">
          ...
          {!address ? (
            ...
            <div className="space-y-4">
              ...
              {contractInfo && (
                <div className="text-sm text-zinc-600 dark:text-zinc-400">
                  컨트랙트: {contractInfo.name} ({contractInfo.symbol})<br></br>
                  컨트랙트 owner : {contractInfo.owner}
                </div>
              )}
            </div>
          )}
